...
class NistCompoundById(object):
    ...
    def _get_data(self, parameter, table=0):
        ...
        prop_table = prop_soup.find_all("table", {"class": "data"})[table].find_all(
            "tr"
        )
    ...
    def get_spectra(self, spectral_type):
        ...
        spectral_defs = {
            "mass": "Mass spectrum (electron ionization)",
        ...
        if spectral_defs.get(spectral_type) is None:
            print(
                (
                    "<Invalid spectral type specified for compound %s. Supported types are 'uv', 'ir' and 'mass'>"
                    % self.name
                )
            )
            return None
        ...
        if spectral_page.status_code != 200:
            print(
                (
                    "<Error downloading web page. Error status code %s>"
                    % (spectral_page.status_code)
                )
            )
            return None
        ...
        if spectral_type == "uv":
            dl_link = (
                "http://webbook.nist.gov/cgi/cbook.cgi?JCAMP=%s&amp;Index=0&amp;Type=UVVis"
                % self.id
            )
        elif spectral_type == "ir":
            # TO--DO: there are other IR spectra as well!
            dl_link = (
                "http://webbook.nist.gov/cgi/cbook.cgi?JCAMP=%s&Index=0&Type=IR"
                % self.id
            )
        elif spectral_type == "mass":
            dl_link = (
                "http://webbook.nist.gov/cgi/cbook.cgi?JCAMP=%s&Index=0&Type=Mass"
                % self.id
            )
    ...
    def list_available_spectra(self):
        return list(self.available_spectra.keys())
...
class NistPlot(object):
    ...
    def __init__(self, cpds_to_plot, spectral_type, style, graph_title=""):
        ...
        if not (spectral_type in ["mass", "uv", "ir"]):
            print(
                "<Invalid spectral type specified %s; mass, ir or uv are valid spectral types>"
                % spectral_type
            )
            return None
        ...
        if isinstance(cpds_to_plot, list):
            ...
            for item in cpds_to_plot:
                if not (isinstance(item, NistCompoundById)):
                    print(
                        "<Object %s is not valid for plotting. Expected type NistCompoundById, got %s>"
                        % (item, type(item))
                    )
                    return None
            ...
            print(
                "<No list of compounds provided to plot. NistPlot supports Python lists or NistCompoundById objects>"
            )
            return None
        ...
        for counter, compound in enumerate(self.compounds):
            if not (isinstance(compound, NistCompoundById)):
                print(
                    "<Compound %s is not a valid NistPy object; got class %s instead>"
            ...
            if compound.spectra[spectral_type] is None:
                ...
                if compound.get_spectra(spectral_type) is None:
                    print(
                        "<Spectrum could not be loaded for compound %s>" % compound.name
                    )
                    continue
    ...
    def show(self):
        ...
        for counter, compound in enumerate(self.compounds):
            ...
            if self.spectral_type == "mass":
                plt.bar(
                    compound.spectra[self.spectral_type]["x"],
                    compound.spectra[self.spectral_type]["y"],
                    0.25,
                    color=self.style[0][0],
                )
            else:
    ...
    def __add__(self, other):
        if not (isinstance(other, (NistPlot, NistCompoundById, list))):
            raise TypeError(
                "<Unsupported operand for concatenation; only NistCompoundById objects, Python lists \
        ...
        if isinstance(other, NistPlot):
            ...
            if self.spectral_type != other.spectral_type:
                print(
                    "Warning: type mismatch detected; one graph is for %s and the other for %s"
            ...
            for cpd in other:
                if isinstance(cpd, NistCompoundById):
                    ...
                    print(
                        "<Some item(s) in supplied list are not valid NistPy object(s). Expected NistCompoundById class, got %s>"
                        % type(cpd)
                    )

        self.title = self.spectral_type.title() + " Spectrum for %s compounds" % len(
            list(set(concat_list))
        )
...
def download_page(url):
    ...
    if page.status_code != 200:
        print(
            "<Error downloading web page. Error status code %s>" % cpd_page.status_code
        )
        return None
    ...
    def instantiate(self):
        ...
        if self.page is None:
            self.page = download_page(
                "http://webbook.nist.gov/cgi/cbook.cgi?ID=%s" % self.id
            )
...
def search_nist(options=None, instantiate=False, **kwargs):
    ...
    search_type, search_str = list(kwargs.keys())[0], list(kwargs.values())[0]
    ...
    if not (search_type) in types:
        print(
            "<Got unexpected search type. Only these search types allowed %s>"
            % list(types.keys())
        )
        return None
    ...
    # there is only one search result!
        ...
        for link in search_page.find_all("a"):
            ...
            if curr_href is not None and re.search(r"ID=([A-Za-z]\d{1,})", curr_href):
                ...
                links_on_page.append(
                    re.search(r"ID=([A-Za-z]\d{1,})", curr_href).group(1)
                )
        ...
        if instantiate == True:
            search_results.append(
                NistResult(id_code, name, souped_page=search_page).instantiate()
            )
        else:
